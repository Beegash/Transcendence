      ...
      <div class="w-full max-w-md">
        <div class="card">
          ...
          <form id="register-form" class="space-y-6">
            <div>
              <label class="block text-sm text-gray-400 mb-2">${t('auth.username')}</label>
              <input type="text" name="username" class="input" placeholder="coolplayer42" required minlength="3" maxlength="20">
            </div>
            
            <div>
              <label class="block text-sm text-gray-400 mb-2">${t('auth.email')}</label>
              <input type="email" name="email" class="input" placeholder="your@email.com" required>
            </div>
            
            <div>
              <label class="block text-sm text-gray-400 mb-2">${t('auth.password')}</label>
              <input type="password" name="password" class="input" placeholder="••••••••" required minlength="8">
              <p class="text-xs text-gray-500 mt-1">Min 8 chars, 1 uppercase, 1 lowercase, 1 number</p>
            </div>
            
            <div>
              <label class="block text-sm text-gray-400 mb-2">${t('auth.confirmPassword')}</label>
          ...
          <div class="my-6 flex items-center">
            <div class="flex-1 border-t border-pong-light"></div>
            <span class="px-4 text-gray-500 text-sm">${t('auth.orContinueWith')}</span>
            <div class="flex-1 border-t border-pong-light"></div>
          </div>
          
          <!-- OAuth Buttons -->
          <div class="space-y-3">
            <a href="/api/auth/oauth/google" class="flex items-center justify-center gap-3 w-full py-3 border border-pong-light rounded-lg hover:bg-pong-light/50 transition-colors">
              ...
              <span class="text-gray-300">${t('auth.loginWithGoogle')}</span>
            </a>
            
            <a href="/api/auth/oauth/github" class="flex items-center justify-center gap-3 w-full py-3 border border-pong-light rounded-lg hover:bg-pong-light/50 transition-colors">
              ...
              <span class="text-gray-300">${t('auth.loginWithGithub')}</span>
            </a>
          </div>
