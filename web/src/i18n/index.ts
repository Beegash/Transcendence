// ft_transcendence - Internationalization (i18n)

export type Language = 'en' | 'tr' | 'de';

interface Translations {
	[key: string]: string | Translations;
}

// English translations
const en: Translations = {
	nav: {
		home: 'Home',
		play: 'Play',
		tournament: 'Tournament',
		dashboard: 'Dashboard',
		profile: 'Profile',
		settings: 'Settings',
		logout: 'Logout',
		notifications: 'Notifications',
	},
	auth: {
		login: 'Log In',
		register: 'Register',
		email: 'Email',
		password: 'Password',
		confirmPassword: 'Confirm Password',
		username: 'Username',
		forgotPassword: 'Forgot Password?',
		noAccount: "Don't have an account?",
		hasAccount: 'Already have an account?',
	},
	home: {
		title: 'Welcome to Pong',
		subtitle: 'The ultimate multiplayer Pong experience',
		playNow: 'Play Now',
		joinTournament: 'Join Tournament',
		features: {
			multiplayer: 'Real-time Multiplayer',
			multiplayerDesc: 'Play against friends or strangers from around the world',
			tournaments: 'Tournaments',
			tournamentsDesc: 'Compete in exciting tournaments and climb the leaderboard',
			ai: 'AI Opponent',
			aiDesc: 'Practice against our challenging AI when no one is around',
		},
	},
	game: {
		title: 'Choose Game Mode',
		localPlay: 'Local Play',
		localPlayDesc: 'Play against a friend on the same device',
		onlinePlay: 'Online Play',
		onlinePlayDesc: 'Play against others online',
		aiPlay: 'vs AI',
		aiPlayDesc: 'Play against an AI opponent',
		aiRefreshInfo: 'AI refreshes view every 1 second',
		waiting: 'Waiting for opponent...',
		ready: 'Ready!',
		start: 'Start Game',
		pause: 'Pause',
		resume: 'Resume',
		quit: 'Quit Game',
		victory: 'Victory!',
		defeat: 'Defeat',
		youWin: 'You Win!',
		youLose: 'You Lose!',
		draw: 'Draw',
		score: 'Score',
		playAgain: 'Play Again',
		rematch: 'Rematch',
		backToMenu: 'Back to Menu',
		you: 'YOU',
		player: 'Player',
		opponent: 'OPPONENT',
		red: 'Red',
		blue: 'Blue',
		firstToWins: 'First to {score} wins',
		useArrows: 'Use ↑/↓ or W/S to move your paddle',
		swipeInstructions: 'Swipe up/down on screen or use W/S - ↑/↓ on keyboard to move your paddle',
		pressSpace: 'Press SPACE to start',
		pressSpaceRestart: 'Press SPACE to restart',
		localControls: 'Player 1: W/S • Player 2: ↑/↓ • Press SPACE to start',
		savingResult: 'Saving result...',
		tournamentMatch: 'Tournament Match',
		matchId: 'Match ID',
		gameInvitation: 'Game Invitation',
		joiningPrivateRoom: 'Joining private room:',
		joiningMatch: 'Connecting to match...',
		connectedJoiningMatch: 'Connected. Joining match...',
		champion: 'CHAMPION',
		roomClosed: 'Room closed. Returning to lobby.',
		opponentDisconnected: 'Opponent disconnected. Returning to lobby.',
		connecting: 'Connecting...',
		connected: 'Connected ✓',
		connectionFailed: 'Connection Failed',
		couldNotConnect: 'Could not connect to game server.',
		opponentJoined: 'Opponent Joined!',
		playingAgainst: 'Playing against:',
		clickReady: "Click Ready when you're prepared to play.",
		clickReadyHint: "Click Ready when you're prepared to play.",
		clickReadyToStart: 'Click Ready to start the game!',
		imReady: "I'm Ready!",
		waitingForOpponent: 'Waiting for opponent...',
		controls: 'Player 1: W/S • Player 2: ↑/↓',
		realTimeMultiplayer: 'Real-time multiplayer',
		langChangeWarning: 'You cannot change the language while in a game lobby. The game will be cancelled.',
		vsAI: 'vs AI',
		connectingServer: 'Connecting to game server...',
		aiReady: 'AI Ready!',
		aiOpponent: 'AI Opponent',
		startingGame: 'Starting game...',
		createRoom: 'Create Room',
		createRoomDesc: 'Create a new room and share the code with your friend.',
		joinRoom: 'Join Room',
		joinRoomDesc: 'Enter a room code to join an existing game.',
		roomCode: 'ROOM CODE',
		join: 'Join',
		availableRooms: 'Available Rooms',
		refresh: 'Refresh',
		loadingRooms: 'Loading rooms...',
		noRooms: 'No rooms available. Create one!',
		failedLoadRooms: 'Failed to load rooms',
		roomCodeError: 'Room code must be 6 characters',
		roomCodeLabel: 'Room Code:',
		copyCode: 'Copy Code',
		copied: 'Copied!',
		waitingPlayer2: 'Waiting for Player 2 to join...',
		youArePlayer: 'You are Player',
	},
	tournament: {
		title: 'Tournaments',
		join: 'Join Tournament',
		start: 'Start Tournament',
		startWithPlayers: 'Start Tournament ({count} players)',
		back: '← Back',
		backToTournaments: 'Back to Tournaments',
		participants: 'Participants',
		noParticipants: 'No participants yet',
		bracket: 'Bracket',
		loadingBracket: 'Loading bracket...',
		clickToJoin: 'Click below to join as',
		yourself: 'yourself',
		playMatch: 'Play Match',
		waiting: 'Waiting',
		inProgress: 'In Progress',
		completed: 'Completed',
		pending: 'Pending',
		semi: 'SEMI',
		final: 'FINAL',
		round: 'Round',
		kickParticipant: 'Kick Participant',
		notFound: 'Tournament not found',
		joinError: 'Could not join tournament. It may be full, already started, or you have already joined.',
		startError: 'Could not start tournament. You may not be the creator or there are not enough players.',
		onlyCreatorCanStart: 'Only the creator can start the tournament',
		createTournament: 'Create Tournament',
		tournamentName: 'Tournament Name',
		maxPlayers: 'Max Players',
		create: 'Create',
		noTournaments: 'No tournaments found',
		noTournamentsYet: 'No tournaments yet. Create one to get started!',
		loadingTournaments: 'Loading tournaments...',
		maxPlayersInfo: 'Max {count} players',
		createdOn: 'Created',
		view: 'View',
		activeTournaments: 'Active Tournaments',
		upcoming: 'Upcoming',
	},
	profile: {
		title: 'Profile',
		stats: 'Statistics',
		matchHistory: 'Match History',
		friends: 'Friends',
		wins: 'Wins',
		losses: 'Losses',
		winRate: 'Win Rate',
		totalGames: 'Total Games',
		editProfile: 'Edit Profile',
		changeAvatar: 'Change Avatar',
		removeAvatar: 'Remove Avatar',
		avatarRemoved: 'Avatar removed successfully',
		addFriend: 'Add Friend',
		online: 'Online',
		offline: 'Offline',
		playing: 'In Game',
		noFriends: 'No friends yet. Play some games to make friends!',
		noMatches: 'No matches played yet. Start playing to see your history!',
		userNotFound: 'User not found',
		goHome: 'Go Home',
		friendRequestPending: 'Request Sent',
		friendRequestSent: 'Friend request sent!',
	},
	friends: {
		findPlayers: 'Find Players',
		searchPlaceholder: 'Search by username...',
		myFriends: 'My Friends',
		searching: 'Searching...',
		sending: 'Sending...',
		sent: 'Sent',
		failedToSend: 'Failed to send request',
		invite: 'Invite',
		inviting: 'Inviting...',
		failedToInvite: 'Failed to send invite',
		errorLoading: 'Error loading friends',
	},
	settings: {
		title: 'Settings',
		language: 'Preferred Language',
		languageHint: 'This will be your default language when you log in.',
		languageSaved: 'Language preference saved!',
		theme: 'Theme',
		privacy: 'Privacy',
		notifications: 'Notifications',
		deleteAccount: 'Delete Account',
		anonymizeAccount: 'Anonymize Account',
		exportData: 'Export My Data',
		gameInvitations: 'Game Invitations',
		tournamentUpdates: 'Tournament Updates',
		friendRequests: 'Friend Requests',
		changeUsername: 'Change Username',
		usernameHint: 'Username must be 3-20 characters, letters, numbers, and underscores only.',
		usernameRequired: 'Please enter a username',
		usernameLengthError: 'Username must be 3-20 characters',
		usernameFormatError: 'Only letters, numbers, and underscores allowed',
		usernameChanged: 'Username changed successfully!',
		changePassword: 'Change Password',
		currentPassword: 'Current Password',
		newPassword: 'New Password',
		confirmNewPassword: 'Confirm New Password',
		passwordHint: 'Password must be at least 8 characters with uppercase, lowercase, and a number.',
		passwordChanged: 'Password changed successfully!',
		passwordIncorrect: 'Current password is incorrect',
		dangerZone: 'Danger Zone',
		dangerZoneWarning: 'Once you delete your account, there is no going back. Please be certain.',
		anonymizeModalTitle: 'Anonymize Account',
		anonymizeModalWarning1: 'This will remove all your personal data (identity, email) but keep your game statistics under an anonymous name.',
		anonymizeModalWarning2: 'You will NOT be able to log in to this account again.',
		anonymizeModalConfirm: 'Type ANONYMIZE to confirm:',
		anonymizeModalPlaceholder: 'Type ANONYMIZE',
		anonymizeSuccess: 'Account anonymized successfully. You have been logged out.',
		anonymizeFailed: 'Failed to anonymize account',
		deleteModalTitle: 'Confirm Account Deletion',
		deleteModalWarning: 'This action cannot be undone. All your data, including match history, stats, and friends list will be permanently deleted.',
		deleteModalConfirm: 'Type DELETE to confirm:',
		deleteModalPlaceholder: 'Type DELETE',
		deleteSuccess: 'Account deleted successfully',
		deleteFailed: 'Failed to delete account',
		processing: 'Processing...',
		deleting: 'Deleting...',
	},
	notifications: {
		title: 'Notifications',
		friendRequest: '{name} sent you a friend request',
		gameInvite: '{name} invited you to play',
		accepted: '{name} accepted your friend request',
		noNotifications: 'No notifications',
		accept: 'Accept',
		decline: 'Decline',
		viewProfile: 'View Profile',
	},
	errors: {
		required: 'This field is required',
		invalidEmail: 'Invalid email address',
		passwordMismatch: 'Passwords do not match',
		minLength: 'Must be at least {min} characters',
		networkError: 'Network error. Please try again.',
		unauthorized: 'Please log in to continue',
		notFound: 'Page not found',
		fileTooLarge: 'File size too large. Maximum file size is 5MB. Please compress or resize your image.',
		invalidFileType: 'Invalid file type. Allowed: JPEG, PNG, GIF, WebP',
		uploadFailed: 'Failed to upload file. Please try again.',
		imageTooLarge: 'Image resolution too large. Maximum dimensions are 2048x2048 pixels. Please resize your image.',
	},
	common: {
		loading: 'Loading...',
		save: 'Save',
		cancel: 'Cancel',
		confirm: 'Confirm',
		delete: 'Delete',
		edit: 'Edit',
		search: 'Search',
		noResults: 'No results found',
		back: 'Back',
		madeBy: 'Made by',
		close: 'Close',
		iAgree: 'I Agree',
		deletedUser: 'Deleted User',
	},
	gdpr: {
		checkboxLabel: 'I have read and agree to the',
		linkText: 'GDPR',
		mustAgree: 'You must agree to the GDPR to register',
		popup: {
			title: 'Your Data Privacy Rights',
			intro: 'We are committed to protecting your personal data and respecting your privacy rights under the GDPR. Below are your rights and how you can exercise them:',
			rightsHeading: 'Your Privacy Rights',
			rights: {
				access: {
					title: 'Right to Access',
					description: 'You have the right to request a copy of your personal data we hold about you.',
				},
				rectification: {
					title: 'Right to Rectification',
					description: 'You can request correction of inaccurate or incomplete personal data.',
				},
				erasure: {
					title: 'Right to Erasure (Right to be Forgotten)',
					description: 'You can request deletion of your personal data when it is no longer necessary for the purposes it was collected.',
				},
				restriction: {
					title: 'Right to Restriction of Processing',
					description: 'You can request that we limit the processing of your personal data under certain circumstances.',
				},
				portability: {
					title: 'Right to Data Portability',
					description: 'You can request to receive your personal data in a structured, commonly used format and transfer it to another service.',
				},
				object: {
					title: 'Right to Object',
					description: 'You can object to the processing of your personal data for direct marketing or other legitimate interests.',
				},
				withdraw: {
					title: 'Right to Withdraw Consent',
					description: 'You can withdraw your consent at any time where processing is based on consent.',
				},
				complain: {
					title: 'Right to Lodge a Complaint',
					description: 'You have the right to lodge a complaint with a supervisory authority if you believe your data protection rights have been violated.',
				},
			},
			dataWeCollect: 'Data We Collect',
			dataList: 'We collect: username, email, game statistics, match history, and profile information. We do not sell your data to third parties.',
			understand: 'I Understand My Rights',
		},
	},
	time: {
		justNow: 'just now',
		minutesAgo: '{count}m ago',
		hoursAgo: '{count}h ago',
		daysAgo: '{count}d ago',
	},
	dashboard: {
		title: 'Dashboard',
		players: 'Players',
		matches: 'Matches',
		tournaments: 'Tournaments',
		today: 'Today',
		active: 'Active',
		winRate: 'Win Rate',
		games: 'Games',
		wins: 'Wins',
		losses: 'Losses',
		pointsScored: 'Points Scored',
		tournamentsPlayed: 'Tournaments Played',
		tournamentsWon: 'Tournaments Won',
		matchHistory: 'Match History',
		recentMatches: 'Recent Matches',
		yourStats: 'Your Stats',
		noStatsYet: 'No stats yet. Play some games!',
		failedLoad: 'Failed to load stats',
		noData: 'No data',
		noMatches: 'No matches played yet',
		leaderboard: 'Leaderboard',
		loadingStats: 'Loading stats...',
		noPlayers: 'No players yet',
		winsVsLosses: 'Wins vs Losses',
		matchTypes: {
			tournament: 'Tournament',
			casual: 'Casual',
			ai: 'vs AI',
			local: 'Local',
			online: 'Online',
		},
	},
};

// Turkish translations
const tr: Translations = {
	nav: {
		home: 'Ana Sayfa',
		play: 'Oyna',
		tournament: 'Turnuva',
		dashboard: 'Panel',
		profile: 'Profil',
		settings: 'Ayarlar',
		logout: 'Çıkış Yap',
		notifications: 'Bildirimler',
	},
	auth: {
		login: 'Giriş Yap',
		register: 'Kayıt Ol',
		email: 'E-posta',
		password: 'Şifre',
		confirmPassword: 'Şifre Tekrar',
		username: 'Kullanıcı Adı',
		forgotPassword: 'Şifremi Unuttum?',
		noAccount: 'Hesabınız yok mu?',
		hasAccount: 'Zaten hesabınız var mı?',
	},
	home: {
		title: "Pong'a Hoş Geldiniz",
		subtitle: 'En iyi çok oyunculu Pong deneyimi',
		playNow: 'Şimdi Oyna',
		joinTournament: 'Turnuvaya Katıl',
		features: {
			multiplayer: 'Gerçek Zamanlı Çok Oyunculu',
			multiplayerDesc: 'Dünyanın dört bir yanından arkadaşlarınıza veya yabancılara karşı oynayın',
			tournaments: 'Turnuvalar',
			tournamentsDesc: 'Heyecan verici turnuvalarda yarışın ve liderlik tablosunda yükselin',
			ai: 'Yapay Zeka Rakibi',
			aiDesc: 'Kimse yokken zorlu yapay zekamıza karşı pratik yapın',
		},
	},
	game: {
		title: 'Oyun Modu Seç',
		localPlay: 'Yerel Oyun',
		localPlayDesc: 'Aynı cihazda bir arkadaşınla oyna',
		onlinePlay: 'Çevrimiçi Oyun',
		onlinePlayDesc: 'Çevrimiçi rakiplere karşı oyna',
		aiPlay: 'Yapay Zekaya Karşı',
		aiPlayDesc: 'Yapay zeka rakibe karşı oyna',
		aiRefreshInfo: 'YZ her 1 saniyede güncellenir',
		waiting: 'Rakip bekleniyor...',
		ready: 'Hazır!',
		start: 'Oyunu Başlat',
		pause: 'Duraklat',
		resume: 'Devam Et',
		quit: 'Oyundan Çık',
		victory: 'Zafer!',
		defeat: 'Yenilgi',
		youWin: 'Kazandın!',
		youLose: 'Kaybettin!',
		draw: 'Berabere',
		score: 'Skor',
		playAgain: 'Tekrar Oyna',
		rematch: 'Rövanş',
		backToMenu: 'Menüye Dön',
		you: 'SEN',
		player: 'Oyuncu',
		opponent: 'RAKİP',
		red: 'Kırmızı',
		blue: 'Mavi',
		firstToWins: 'İlk {score} sayı kazanır',
		useArrows: 'Hareket: ↑/↓ veya W/S',
		swipeInstructions: 'Ekranda yukarı/aşağı kaydırın veya klavyede W/S - ↑/↓ tuşlarını kullanarak paddle\'ınızı hareket ettirin',
		pressSpace: 'Başlatmak için SPACE basın',
		pressSpaceRestart: 'Başlatmak için SPACE basın',
		localControls: 'Oyuncu 1: W/S • Oyuncu 2: ↑/↓ • Başlatmak için SPACE basın',
		savingResult: 'Sonuç kaydediliyor...',
		tournamentMatch: 'Turnuva Maçı',
		matchId: 'Maç ID',
		gameInvitation: 'Oyun Daveti',
		joiningPrivateRoom: 'Özel odaya katılıyor:',
		joiningMatch: 'Maça bağlanıyor...',
		connectedJoiningMatch: 'Bağlandı. Maça katılıyor...',
		champion: 'ŞAMPİYON',
		roomClosed: 'Oda kapandı. Lobiye dönülüyor.',
		opponentDisconnected: 'Rakip bağlantısını kaybetti. Lobiye dönülüyor.',
		connecting: 'Bağlanıyor...',
		connected: 'Bağlandı',
		connectionFailed: 'Bağlantı Başarısız',
		couldNotConnect: 'Oyun sunucusuna bağlanılamadı.',
		opponentJoined: 'Rakip Katıldı!',
		playingAgainst: 'Rakibiniz:',
		clickReady: "Hazır olduğunuzda Hazırım'a tıklayın.",
		clickReadyHint: "Hazır olduğunuzda Hazırım'a tıklayın.",
		clickReadyToStart: "Oyunu başlatmak için Hazırım'a tıklayın!",
		imReady: 'Hazırım!',
		waitingForOpponent: 'Rakip bekleniyor...',
		controls: 'Oyuncu 1: W/S • Oyuncu 2: ↑/↓',
		realTimeMultiplayer: 'Gerçek zamanlı çok oyunculu',
		langChangeWarning: 'Oyun lobisindeyken dil değiştirilemez. Oyun iptal edilecektir.',
		vsAI: 'Yapay Zekaya Karşı',
		connectingServer: 'Oyun sunucusuna bağlanılıyor...',
		aiReady: 'Yapay Zeka Hazır!',
		aiOpponent: 'Yapay Zeka Rakibi',
		startingGame: 'Oyun başlatılıyor...',
		createRoom: 'Oda Oluştur',
		createRoomDesc: 'Yeni bir oda oluşturun ve kodu arkadaşınızla paylaşın.',
		joinRoom: 'Odaya Katıl',
		joinRoomDesc: 'Mevcut bir oyuna katılmak için oda kodunu girin.',
		roomCode: 'ODA KODU',
		join: 'Katıl',
		availableRooms: 'Mevcut Odalar',
		refresh: 'Yenile',
		loadingRooms: 'Odalar yükleniyor...',
		noRooms: 'Mevcut oda yok. Bir tane oluşturun!',
		failedLoadRooms: 'Odalar yüklenemedi',
		roomCodeError: 'Oda kodu 6 karakter olmalıdır',
		roomCodeLabel: 'Oda Kodu:',
		copyCode: 'Kodu Kopyala',
		copied: 'Kopyalandı!',
		waitingPlayer2: "Oyuncu 2'nin katılması bekleniyor...",
		youArePlayer: 'Siz Oyuncu',
	},
	tournament: {
		title: 'Turnuvalar',
		join: 'Turnuvaya Katıl',
		start: 'Turnuvayı Başlat',
		startWithPlayers: 'Turnuvayı Başlat ({count} oyuncu)',
		back: '← Geri',
		backToTournaments: 'Turnuvalara Dön',
		participants: 'Katılımcılar',
		noParticipants: 'Henüz katılımcı yok',
		bracket: 'Eşleşmeler',
		loadingBracket: 'Eşleşmeler yükleniyor...',
		clickToJoin: 'Katılmak için tıklayın',
		yourself: 'kendiniz',
		playMatch: 'Maçı Oyna',
		waiting: 'Bekliyor',
		inProgress: 'Devam Ediyor',
		completed: 'Tamamlandı',
		pending: 'Beklemede',
		semi: 'YARI FİNAL',
		final: 'FİNAL',
		round: 'Tur',
		kickParticipant: 'Katılımcıyı Çıkar',
		notFound: 'Turnuva bulunamadı',
		joinError: 'Turnuvaya katılınamadı. Dolu olabilir, başlamış olabilir veya zaten katılmış olabilirsiniz.',
		startError: 'Turnuva başlatılamadı. Oluşturan siz olmayabilirsiniz veya yeterli oyuncu yok.',
		onlyCreatorCanStart: 'Turnuvayı sadece oluşturan başlatabilir',
		createTournament: 'Turnuva Oluştur',
		tournamentName: 'Turnuva Adı',
		maxPlayers: 'Maksimum Oyuncu',
		create: 'Oluştur',
		noTournaments: 'Turnuva bulunamadı',
		noTournamentsYet: 'Henüz turnuva yok. Başlamak için bir tane oluşturun!',
		loadingTournaments: 'Turnuvalar yükleniyor...',
		maxPlayersInfo: 'Maksimum {count} oyuncu',
		createdOn: 'Oluşturulma',
		view: 'Görüntüle',
		activeTournaments: 'Aktif Turnuvalar',
		upcoming: 'Yaklaşan',
	},
	profile: {
		title: 'Profil',
		stats: 'İstatistikler',
		matchHistory: 'Maç Geçmişi',
		friends: 'Arkadaşlar',
		wins: 'Galibiyet',
		losses: 'Mağlubiyet',
		winRate: 'Kazanma Oranı',
		totalGames: 'Toplam Oyun',
		editProfile: 'Profili Düzenle',
		changeAvatar: 'Avatar Değiştir',
		removeAvatar: 'Fotoğrafı Kaldır',
		avatarRemoved: 'Fotoğraf başarıyla kaldırıldı',
		addFriend: 'Arkadaş Ekle',
		online: 'Çevrimiçi',
		offline: 'Çevrimdışı',
		playing: 'Oyunda',
		noFriends: 'Henüz arkadaşın yok. Oyun oynayarak arkadaş edin!',
		noMatches: 'Henüz maç oynamadın. Geçmişini görmek için oynamaya başla!',
		userNotFound: 'Kullanıcı bulunamadı',
		goHome: 'Ana Sayfaya Git',
		friendRequestPending: 'İstek Gönderildi',
		friendRequestSent: 'Arkadaşlık isteği gönderildi!',
	},
	friends: {
		findPlayers: 'Oyuncu Bul',
		searchPlaceholder: 'Kullanıcı adına göre ara...',
		myFriends: 'Arkadaşlarım',
		searching: 'Aranıyor...',
		sending: 'Gönderiliyor...',
		sent: 'Gönderildi',
		failedToSend: 'İstek gönderilemedi',
		invite: 'Davet Et',
		inviting: 'Davet ediliyor...',
		failedToInvite: 'Davet gönderilemedi',
		errorLoading: 'Arkadaşlar yüklenirken hata oluştu',
	},
	settings: {
		title: 'Ayarlar',
		language: 'Tercih Edilen Dil',
		languageHint: 'Bu, giriş yaptığınızda varsayılan diliniz olacaktır.',
		languageSaved: 'Dil tercihi kaydedildi!',
		theme: 'Tema',
		privacy: 'Gizlilik',
		notifications: 'Bildirimler',
		deleteAccount: 'Hesabı Sil',
		anonymizeAccount: 'Hesabı Anonimleştir',
		exportData: 'Verilerimi İndir',
		gameInvitations: 'Oyun Davetleri',
		tournamentUpdates: 'Turnuva Güncellemeleri',
		friendRequests: 'Arkadaşlık İstekleri',
		changeUsername: 'Kullanıcı Adı Değiştir',
		usernameHint: 'Kullanıcı adı 3-20 karakter, harf, rakam ve _, - içerebilir.',
		usernameRequired: 'Lütfen bir kullanıcı adı girin',
		usernameLengthError: 'Kullanıcı adı 3-20 karakter olmalıdır',
		usernameFormatError: 'Sadece harf, rakam ve alt çizgi kullanılabilir',
		usernameChanged: 'Kullanıcı adı başarıyla değiştirildi!',
		changePassword: 'Şifre Değiştir',
		currentPassword: 'Mevcut Şifre',
		newPassword: 'Yeni Şifre',
		confirmNewPassword: 'Yeni Şifre (Tekrar)',
		passwordHint: 'Şifre en az 8 karakter, büyük harf, küçük harf ve rakam içermelidir.',
		passwordChanged: 'Şifre başarıyla değiştirildi!',
		passwordIncorrect: 'Mevcut şifre yanlış',
		dangerZone: 'Tehlikeli Bölge',
		dangerZoneWarning: 'Hesabınızı sildikten sonra geri dönüşü yoktur. Lütfen emin olun.',
		anonymizeModalTitle: 'Hesabı Anonimleştir',
		anonymizeModalWarning1: 'Bu işlem tüm kişisel verilerinizi (kimlik, e-posta) kaldıracak ancak oyun istatistiklerinizi anonim bir isim altında tutacaktır.',
		anonymizeModalWarning2: 'Bu hesaba bir daha giriş YAPAMAYACAKSINIZ.',
		anonymizeModalConfirm: 'Onaylamak için ANONYMIZE yazın:',
		anonymizeModalPlaceholder: 'ANONYMIZE yazın',
		anonymizeSuccess: 'Hesap başarıyla anonimleştirildi. Çıkış yapıldı.',
		anonymizeFailed: 'Hesap anonimleştirilemedi',
		deleteModalTitle: 'Hesap Silme Onayı',
		deleteModalWarning: 'Bu işlem geri alınamaz. Maç geçmişi, istatistikler ve arkadaş listeniz dahil tüm verileriniz kalıcı olarak silinecektir.',
		deleteModalConfirm: 'Onaylamak için DELETE yazın:',
		deleteModalPlaceholder: 'DELETE yazın',
		deleteSuccess: 'Hesap başarıyla silindi',
		deleteFailed: 'Hesap silinemedi',
		processing: 'İşleniyor...',
		deleting: 'Siliniyor...',
	},
	notifications: {
		title: 'Bildirimler',
		friendRequest: '{name} size arkadaşlık isteği gönderdi',
		gameInvite: '{name} sizi oyuna davet etti',
		accepted: '{name} arkadaşlık isteğinizi kabul etti',
		noNotifications: 'Bildirim yok',
		accept: 'Kabul Et',
		decline: 'Reddet',
		viewProfile: 'Profili Gör',
	},
	errors: {
		required: 'Bu alan zorunludur',
		invalidEmail: 'Geçersiz e-posta adresi',
		passwordMismatch: 'Şifreler eşleşmiyor',
		minLength: 'En az {min} karakter olmalıdır',
		networkError: 'Ağ hatası. Lütfen tekrar deneyin.',
		unauthorized: 'Devam etmek için giriş yapın',
		notFound: 'Sayfa bulunamadı',
		fileTooLarge: 'Dosya boyutu çok büyük. Maksimum dosya boyutu 5MB. Lütfen resminizi sıkıştırın veya yeniden boyutlandırın.',
		invalidFileType: 'Geçersiz dosya türü. İzin verilenler: JPEG, PNG, GIF, WebP',
		uploadFailed: 'Dosya yüklenemedi. Lütfen tekrar deneyin.',
		imageTooLarge: 'Resim çözünürlüğü çok yüksek. Maksimum boyut 2048x2048 piksel. Lütfen resminizi yeniden boyutlandırın.',
	},
	common: {
		loading: 'Yükleniyor...',
		save: 'Kaydet',
		cancel: 'İptal',
		confirm: 'Onayla',
		delete: 'Sil',
		edit: 'Düzenle',
		search: 'Ara',
		noResults: 'Sonuç bulunamadı',
		back: 'Geri',
		madeBy: 'Yapımcılar',
		close: 'Kapat',
		iAgree: 'Kabul Ediyorum',
		deletedUser: 'Silinmiş Kullanıcı',
	},
	gdpr: {
		checkboxLabel: 'Okudum ve kabul ediyorum:',
		linkText: 'GDPR',
		mustAgree: 'Kayıt olmak için GDPR\'ı kabul etmelisiniz',
		popup: {
			title: 'Veri Gizliliği Haklarınız',
			intro: 'Kişisel verilerinizi korumaya ve GDPR kapsamındaki gizlilik haklarınıza saygı göstermeye kararlıyız. Aşağıda haklarınız ve bunları nasıl kullanabileceğiniz yer almaktadır:',
			rightsHeading: 'Gizlilik Haklarınız',
			rights: {
				access: {
					title: 'Erişim Hakkı',
					description: 'Hakkınızda tuttuğumuz kişisel verilerin bir kopyasını talep etme hakkına sahipsiniz.',
				},
				rectification: {
					title: 'Düzeltme Hakkı',
					description: 'Yanlış veya eksik kişisel verilerin düzeltilmesini talep edebilirsiniz.',
				},
				erasure: {
					title: 'Silme Hakkı (Unutulma Hakkı)',
					description: 'Kişisel verilerinizin toplandığı amaçlar için artık gerekli olmadığında silinmesini talep edebilirsiniz.',
				},
				restriction: {
					title: 'İşlemenin Kısıtlanması Hakkı',
					description: 'Belirli durumlarda kişisel verilerinizin işlenmesini sınırlandırmamızı talep edebilirsiniz.',
				},
				portability: {
					title: 'Veri Taşınabilirliği Hakkı',
					description: 'Kişisel verilerinizi yapılandırılmış, yaygın kullanılan bir formatta almayı ve başka bir hizmete aktarmayı talep edebilirsiniz.',
				},
				object: {
					title: 'İtiraz Etme Hakkı',
					description: 'Doğrudan pazarlama veya diğer meşru menfaatler için kişisel verilerinizin işlenmesine itiraz edebilirsiniz.',
				},
				withdraw: {
					title: 'Rızayı Geri Çekme Hakkı',
					description: 'İşleme rızaya dayalı olduğunda, rızanızı istediğiniz zaman geri çekebilirsiniz.',
				},
				complain: {
					title: 'Şikayette Bulunma Hakkı',
					description: 'Veri koruma haklarınızın ihlal edildiğine inanıyorsanız, denetim otoritesine şikayette bulunma hakkına sahipsiniz.',
				},
			},
			dataWeCollect: 'Topladığımız Veriler',
			dataList: 'Topladığımız veriler: kullanıcı adı, e-posta, oyun istatistikleri, maç geçmişi ve profil bilgileri. Verilerinizi üçüncü taraflara satmıyoruz.',
			understand: 'Haklarımı Anladım',
		},
	},
	time: {
		justNow: 'şimdi',
		minutesAgo: '{count} dk önce',
		hoursAgo: '{count} sa önce',
		daysAgo: '{count} gün önce',
	},
	dashboard: {
		title: 'Panel',
		players: 'Oyuncular',
		matches: 'Maçlar',
		tournaments: 'Turnuvalar',
		today: 'Bugün',
		active: 'Aktif',
		winRate: 'Kazanma Oranı',
		games: 'Oyunlar',
		wins: 'Galibiyetler',
		losses: 'Mağlubiyetler',
		pointsScored: 'Atılan Sayı',
		tournamentsPlayed: 'Oynanan Turnuva',
		tournamentsWon: 'Kazanılan Turnuva',
		matchHistory: 'Maç Geçmişi',
		recentMatches: 'Son Maçlar',
		yourStats: 'İstatistikleriniz',
		noStatsYet: 'Henüz istatistik yok. Biraz oyun oynayın!',
		failedLoad: 'İstatistikler yüklenemedi',
		noData: 'Veri yok',
		noMatches: 'Henüz maç yapılmadı',
		leaderboard: 'Liderlik Tablosu',
		loadingStats: 'İstatistikler yükleniyor...',
		noPlayers: 'Henüz oyuncu yok',
		winsVsLosses: 'Galibiyet - Mağlubiyet',
		matchTypes: {
			tournament: 'Turnuva',
			casual: 'Normal',
			ai: 'Yapay Zeka',
			local: 'Yerel',
			online: 'Çevrimiçi',
		},
	},
};

// German translations
const de: Translations = {
	nav: {
		home: 'Startseite',
		play: 'Spielen',
		tournament: 'Turnier',
		dashboard: 'Dashboard',
		profile: 'Profil',
		settings: 'Einstellungen',
		logout: 'Abmelden',
		notifications: 'Benachrichtigungen',
	},
	auth: {
		login: 'Anmelden',
		register: 'Registrieren',
		email: 'E-Mail',
		password: 'Passwort',
		confirmPassword: 'Passwort bestätigen',
		username: 'Benutzername',
		forgotPassword: 'Passwort vergessen?',
		noAccount: 'Noch kein Konto?',
		hasAccount: 'Bereits ein Konto?',
	},
	home: {
		title: 'Willkommen bei Pong',
		subtitle: 'Das ultimative Multiplayer-Pong-Erlebnis',
		playNow: 'Jetzt spielen',
		joinTournament: 'Turnier beitreten',
		features: {
			multiplayer: 'Echtzeit-Multiplayer',
			multiplayerDesc: 'Spiele gegen Freunde oder Fremde aus aller Welt',
			tournaments: 'Turniere',
			tournamentsDesc: 'Nimm an spannenden Turnieren teil und steige in der Bestenliste auf',
			ai: 'KI-Gegner',
			aiDesc: 'Übe gegen unsere herausfordernde KI, wenn niemand da ist',
		},
	},
	game: {
		title: 'Spielmodus wählen',
		localPlay: 'Lokales Spiel',
		localPlayDesc: 'Spiele gegen einen Freund auf demselben Gerät',
		onlinePlay: 'Online-Spiel',
		onlinePlayDesc: 'Spiele online gegen andere',
		aiPlay: 'gegen KI',
		aiPlayDesc: 'Spiele gegen einen KI-Gegner',
		aiRefreshInfo: 'KI aktualisiert Ansicht jede Sekunde',
		waiting: 'Warte auf Gegner...',
		ready: 'Bereit!',
		start: 'Spiel starten',
		pause: 'Pause',
		resume: 'Fortsetzen',
		quit: 'Spiel beenden',
		victory: 'Sieg!',
		defeat: 'Niederlage',
		youWin: 'Du gewinnst!',
		youLose: 'Du verlierst!',
		draw: 'Unentschieden',
		score: 'Punktestand',
		playAgain: 'Nochmal spielen',
		rematch: 'Revanche',
		backToMenu: 'Zurück zum Menü',
		you: 'DU',
		player: 'Spieler',
		opponent: 'GEGNER',
		red: 'Rot',
		blue: 'Blau',
		firstToWins: 'Erster mit {score} gewinnt',
		useArrows: 'Benutze ↑/↓ oder W/S um dein Paddel zu bewegen',
		swipeInstructions: 'Wischen Sie auf dem Bildschirm nach oben/unten oder verwenden Sie W/S - ↑/↓ auf der Tastatur, um Ihr Paddel zu bewegen',
		pressSpace: 'Drücken Sie die Leertaste zum Starten',
		pressSpaceRestart: 'Drücken Sie die Leertaste zum Neustarten',
		localControls: 'Spieler 1: W/S • Spieler 2: ↑/↓ • Leertaste zum Starten',
		savingResult: 'Ergebnis wird gespeichert...',
		tournamentMatch: 'Turnierspiel',
		matchId: 'Spiel-ID',
		gameInvitation: 'Spieleinladung',
		joiningPrivateRoom: 'Privatem Raum beitreten:',
		joiningMatch: 'Verbindung zum Spiel wird hergestellt...',
		connectedJoiningMatch: 'Verbunden. Spiel beitreten...',
		champion: 'CHAMPION',
		roomClosed: 'Raum geschlossen. Kehre zur Lobby zurück.',
		opponentDisconnected: 'Gegner getrennt. Kehre zur Lobby zurück.',
		connecting: 'Verbindung wird hergestellt...',
		connected: 'Verbunden ✓',
		connectionFailed: 'Verbindung Fehlgeschlagen',
		couldNotConnect: 'Verbindung zum Spielserver konnte nicht hergestellt werden.',
		opponentJoined: 'Gegner beigetreten!',
		playingAgainst: 'Spielen gegen:',
		clickReady: 'Klicke auf Bereit, wenn du bereit bist zu spielen.',
		clickReadyHint: 'Klicke auf Bereit, wenn du bereit bist zu spielen.',
		clickReadyToStart: 'Klicke auf Bereit, um das Spiel zu starten!',
		imReady: 'Ich bin bereit!',
		waitingForOpponent: 'Warten auf Gegner...',
		controls: 'Spieler 1: W/S • Spieler 2: ↑/↓',
		realTimeMultiplayer: 'Echtzeit-Mehrspielermodus',
		langChangeWarning: 'Die Sprache kann nicht geändert werden, während du in einer Spiellobby bist.',
		vsAI: 'gegen KI',
		connectingServer: 'Verbindung zum Spielserver wird hergestellt...',
		aiReady: 'KI Bereit!',
		aiOpponent: 'KI-Gegner',
		startingGame: 'Spiel startet...',
		createRoom: 'Raum erstellen',
		createRoomDesc: 'Erstelle einen neuen Raum und teile den Code mit deinem Freund.',
		joinRoom: 'Raum beitreten',
		joinRoomDesc: 'Gib einen Raumcode ein, um einem bestehenden Spiel beizutreten.',
		roomCode: 'RAUM-CODE',
		join: 'Beitreten',
		availableRooms: 'Verfügbare Räume',
		refresh: 'Aktualisieren',
		loadingRooms: 'Räume werden geladen...',
		noRooms: 'Keine Räume verfügbar. Erstelle einen!',
		failedLoadRooms: 'Räume konnten nicht geladen werden',
		roomCodeError: 'Raumcode muss 6 Zeichen haben',
		roomCodeLabel: 'Raum-Code:',
		copyCode: 'Code kopieren',
		copied: 'Kopiert!',
		waitingPlayer2: 'Warte auf Spieler 2...',
		youArePlayer: 'Du bist Spieler',
	},
	tournament: {
		title: 'Turniere',
		join: 'Turnier beitreten',
		start: 'Turnier starten',
		startWithPlayers: 'Turnier starten ({count} Spieler)',
		back: '← Zurück',
		backToTournaments: 'Zurück zu Turnieren',
		participants: 'Teilnehmer',
		noParticipants: 'Noch keine Teilnehmer',
		bracket: 'Spielplan',
		loadingBracket: 'Spielplan wird geladen...',
		clickToJoin: 'Klicke unten um beizutreten als',
		yourself: 'du selbst',
		playMatch: 'Spiel spielen',
		waiting: 'Wartend',
		inProgress: 'Läuft',
		completed: 'Abgeschlossen',
		pending: 'Ausstehend',
		semi: 'HALB',
		final: 'FINALE',
		round: 'Runde',
		kickParticipant: 'Teilnehmer entfernen',
		notFound: 'Turnier nicht gefunden',
		joinError: 'Turnier beitreten nicht möglich. Es könnte voll sein, bereits gestartet, oder du bist bereits dabei.',
		startError: 'Turnier starten nicht möglich. Du bist vielleicht nicht der Ersteller oder es sind nicht genug Spieler.',
		onlyCreatorCanStart: 'Nur der Ersteller kann das Turnier starten',
		createTournament: 'Turnier erstellen',
		tournamentName: 'Turniername',
		maxPlayers: 'Max. Spieler',
		create: 'Erstellen',
		noTournaments: 'Keine Turniere gefunden',
		noTournamentsYet: 'Noch keine Turniere. Erstelle eines um zu beginnen!',
		loadingTournaments: 'Turniere werden geladen...',
		maxPlayersInfo: 'Max {count} Spieler',
		createdOn: 'Erstellt',
		view: 'Ansehen',
		activeTournaments: 'Aktive Turniere',
		upcoming: 'Kommende',
	},
	profile: {
		title: 'Profil',
		stats: 'Statistiken',
		matchHistory: 'Spielverlauf',
		friends: 'Freunde',
		wins: 'Siege',
		losses: 'Niederlagen',
		winRate: 'Siegquote',
		totalGames: 'Gesamtspiele',
		editProfile: 'Profil bearbeiten',
		changeAvatar: 'Avatar ändern',
		removeAvatar: 'Avatar entfernen',
		avatarRemoved: 'Avatar erfolgreich entfernt',
		addFriend: 'Freund hinzufügen',
		online: 'Online',
		offline: 'Offline',
		playing: 'Im Spiel',
		noFriends: 'Noch keine Freunde. Spiele um Freunde zu finden!',
		noMatches: 'Noch keine Spiele. Fange an zu spielen um deinen Verlauf zu sehen!',
		userNotFound: 'Benutzer nicht gefunden',
		goHome: 'Zur Startseite',
		friendRequestPending: 'Anfrage gesendet',
		friendRequestSent: 'Freundschaftsanfrage gesendet!',
	},
	friends: {
		findPlayers: 'Spieler finden',
		searchPlaceholder: 'Nach Benutzername suchen...',
		myFriends: 'Meine Freunde',
		searching: 'Suche läuft...',
		sending: 'Wird gesendet...',
		sent: 'Gesendet',
		failedToSend: 'Anfrage konnte nicht gesendet werden',
		invite: 'Einladen',
		inviting: 'Einladung wird gesendet...',
		failedToInvite: 'Einladung konnte nicht gesendet werden',
		errorLoading: 'Fehler beim Laden der Freunde',
	},
	settings: {
		title: 'Einstellungen',
		language: 'Bevorzugte Sprache',
		languageHint: 'Dies ist Ihre Standardsprache bei der Anmeldung.',
		languageSaved: 'Spracheinstellung gespeichert!',
		theme: 'Design',
		privacy: 'Datenschutz',
		notifications: 'Benachrichtigungen',
		deleteAccount: 'Konto löschen',
		anonymizeAccount: 'Konto anonymisieren',
		exportData: 'Meine Daten exportieren',
		gameInvitations: 'Spieleinladungen',
		tournamentUpdates: 'Turnieraktualisierungen',
		friendRequests: 'Freundschaftsanfragen',
		changeUsername: 'Benutzername ändern',
		usernameHint: 'Benutzername muss 3-20 Zeichen lang sein und darf nur Buchstaben, Zahlen und Unterstriche enthalten.',
		usernameRequired: 'Bitte gib einen Benutzernamen ein',
		usernameLengthError: 'Benutzername muss 3-20 Zeichen lang sein',
		usernameFormatError: 'Nur Buchstaben, Zahlen und Unterstriche erlaubt',
		usernameChanged: 'Benutzername erfolgreich geändert!',
		changePassword: 'Passwort ändern',
		currentPassword: 'Aktuelles Passwort',
		newPassword: 'Neues Passwort',
		confirmNewPassword: 'Neues Passwort bestätigen',
		passwordHint: 'Passwort muss mindestens 8 Zeichen mit Großbuchstaben, Kleinbuchstaben und einer Zahl haben.',
		passwordChanged: 'Passwort erfolgreich geändert!',
		passwordIncorrect: 'Aktuelles Passwort ist falsch',
		dangerZone: 'Gefahrenzone',
		dangerZoneWarning: 'Wenn du dein Konto löschst, gibt es kein Zurück. Bitte sei dir sicher.',
		anonymizeModalTitle: 'Konto Anonymisieren',
		anonymizeModalWarning1: 'Dies entfernt alle Ihre persönlichen Daten (Identität, E-Mail), behält aber Ihre Spielstatistiken unter einem anonymen Namen.',
		anonymizeModalWarning2: 'Sie können sich NICHT mehr in dieses Konto einloggen.',
		anonymizeModalConfirm: 'Geben Sie ANONYMIZE ein, um zu bestätigen:',
		anonymizeModalPlaceholder: 'ANONYMIZE eingeben',
		anonymizeSuccess: 'Konto erfolgreich anonymisiert. Sie wurden abgemeldet.',
		anonymizeFailed: 'Konto konnte nicht anonymisiert werden',
		deleteModalTitle: 'Kontolöschung Bestätigen',
		deleteModalWarning: 'Diese Aktion kann nicht rückgängig gemacht werden. Alle Ihre Daten, einschließlich Spielverlauf, Statistiken und Freundesliste, werden dauerhaft gelöscht.',
		deleteModalConfirm: 'Geben Sie DELETE ein, um zu bestätigen:',
		deleteModalPlaceholder: 'DELETE eingeben',
		deleteSuccess: 'Konto erfolgreich gelöscht',
		deleteFailed: 'Konto konnte nicht gelöscht werden',
		processing: 'Wird verarbeitet...',
		deleting: 'Wird gelöscht...',
	},
	notifications: {
		title: 'Benachrichtigungen',
		friendRequest: '{name} hat dir eine Freundschaftsanfrage gesendet',
		gameInvite: '{name} hat dich zum Spielen eingeladen',
		accepted: '{name} hat deine Freundschaftsanfrage angenommen',
		noNotifications: 'Keine Benachrichtigungen',
		accept: 'Annehmen',
		decline: 'Ablehnen',
		viewProfile: 'Profil ansehen',
	},
	errors: {
		required: 'Dieses Feld ist erforderlich',
		invalidEmail: 'Ungültige E-Mail-Adresse',
		passwordMismatch: 'Passwörter stimmen nicht überein',
		minLength: 'Muss mindestens {min} Zeichen haben',
		networkError: 'Netzwerkfehler. Bitte versuche es erneut.',
		unauthorized: 'Bitte melde dich an um fortzufahren',
		notFound: 'Seite nicht gefunden',
		fileTooLarge: 'Dateigröße zu groß. Maximale Dateigröße ist 5MB. Bitte komprimieren oder verkleinern Sie Ihr Bild.',
		invalidFileType: 'Ungültiger Dateityp. Erlaubt: JPEG, PNG, GIF, WebP',
		uploadFailed: 'Datei konnte nicht hochgeladen werden. Bitte versuchen Sie es erneut.',
		imageTooLarge: 'Bildauflösung zu groß. Maximale Abmessungen sind 2048x2048 Pixel. Bitte verkleinern Sie Ihr Bild.',
	},
	common: {
		loading: 'Lädt...',
		save: 'Speichern',
		cancel: 'Abbrechen',
		confirm: 'Bestätigen',
		delete: 'Löschen',
		edit: 'Bearbeiten',
		search: 'Suchen',
		noResults: 'Keine Ergebnisse gefunden',
		back: 'Zurück',
		madeBy: 'Erstellt von',
		close: 'Schließen',
		iAgree: 'Ich stimme zu',
		deletedUser: 'Gelöschter Benutzer',
	},
	gdpr: {
		checkboxLabel: 'Ich habe gelesen und stimme zu:',
		linkText: 'GDPR',
		mustAgree: 'Sie müssen dem GDPR zustimmen, um sich zu registrieren',
		popup: {
			title: 'Ihre Datenschutzrechte',
			intro: 'Wir verpflichten uns, Ihre personenbezogenen Daten zu schützen und Ihre Datenschutzrechte gemäß der DSGVO zu respektieren. Nachfolgend finden Sie Ihre Rechte und wie Sie diese ausüben können:',
			rightsHeading: 'Ihre Datenschutzrechte',
			rights: {
				access: {
					title: 'Recht auf Auskunft',
					description: 'Sie haben das Recht, eine Kopie Ihrer personenbezogenen Daten anzufordern, die wir über Sie gespeichert haben.',
				},
				rectification: {
					title: 'Recht auf Berichtigung',
					description: 'Sie können die Korrektur unrichtiger oder unvollständiger personenbezogener Daten verlangen.',
				},
				erasure: {
					title: 'Recht auf Löschung (Recht auf Vergessenwerden)',
					description: 'Sie können die Löschung Ihrer personenbezogenen Daten verlangen, wenn diese für die Zwecke, für die sie erhoben wurden, nicht mehr erforderlich sind.',
				},
				restriction: {
					title: 'Recht auf Einschränkung der Verarbeitung',
					description: 'Sie können unter bestimmten Umständen verlangen, dass wir die Verarbeitung Ihrer personenbezogenen Daten einschränken.',
				},
				portability: {
					title: 'Recht auf Datenübertragbarkeit',
					description: 'Sie können verlangen, Ihre personenbezogenen Daten in einem strukturierten, gängigen Format zu erhalten und an einen anderen Dienst zu übertragen.',
				},
				object: {
					title: 'Widerspruchsrecht',
					description: 'Sie können der Verarbeitung Ihrer personenbezogenen Daten für Direktwerbung oder andere berechtigte Interessen widersprechen.',
				},
				withdraw: {
					title: 'Recht auf Widerruf der Einwilligung',
					description: 'Sie können Ihre Einwilligung jederzeit widerrufen, wenn die Verarbeitung auf Einwilligung basiert.',
				},
				complain: {
					title: 'Recht auf Beschwerde',
					description: 'Sie haben das Recht, bei einer Aufsichtsbehörde Beschwerde einzulegen, wenn Sie glauben, dass Ihre Datenschutzrechte verletzt wurden.',
				},
			},
			dataWeCollect: 'Daten, die wir sammeln',
			dataList: 'Wir sammeln: Benutzername, E-Mail, Spielstatistiken, Spielverlauf und Profilinformationen. Wir verkaufen Ihre Daten nicht an Dritte.',
			understand: 'Ich verstehe meine Rechte',
		},
	},
	time: {
		justNow: 'gerade eben',
		minutesAgo: 'vor {count} Min',
		hoursAgo: 'vor {count} Std',
		daysAgo: 'vor {count} Tag(en)',
	},
	dashboard: {
		title: 'Dashboard',
		players: 'Spieler',
		matches: 'Spiele',
		tournaments: 'Turniere',
		today: 'Heute',
		active: 'Aktiv',
		winRate: 'Siegquote',
		games: 'Spiele',
		wins: 'Siege',
		losses: 'Niederlagen',
		pointsScored: 'Erzielte Punkte',
		tournamentsPlayed: 'Gespielte Turniere',
		tournamentsWon: 'Gewonnene Turniere',
		matchHistory: 'Spielverlauf',
		recentMatches: 'Letzte Spiele',
		yourStats: 'Deine Statistiken',
		noStatsYet: 'Noch keine Statistiken. Spiele ein paar Spiele!',
		failedLoad: 'Statistiken konnten nicht geladen werden',
		noData: 'Keine Daten',
		noMatches: 'Noch keine Spiele gespielt',
		leaderboard: 'Bestenliste',
		loadingStats: 'Statistiken werden geladen...',
		noPlayers: 'Noch keine Spieler',
		winsVsLosses: 'Siege vs Niederlagen',
		matchTypes: {
			tournament: 'Turnier',
			casual: 'Freundschaftsspiel',
			ai: 'vs KI',
			local: 'Lokal',
			online: 'Online',
		},
	},
};

const translations: Record<Language, Translations> = { en, tr, de };

class I18n {
	private currentLanguage: Language = 'en';
	private listeners: Set<() => void> = new Set();

	constructor() {
		// Load saved language from localStorage
		const saved = localStorage.getItem('language') as Language;
		if (saved && translations[saved]) {
			this.currentLanguage = saved;
		}
	}

	/**
	 * Get current language
	 */
	getLanguage(): Language {
		return this.currentLanguage;
	}

	/**
	 * Set language
	 */
	setLanguage(lang: Language): void {
		if (translations[lang]) {
			this.currentLanguage = lang;
			localStorage.setItem('language', lang);
			document.documentElement.lang = lang;
			this.notifyListeners();
		}
	}

	/**
	 * Translate a key
	 */
	t(key: string, params?: Record<string, string | number>): string {
		const keys = key.split('.');
		let value: string | Translations = translations[this.currentLanguage];

		for (const k of keys) {
			if (typeof value === 'object' && value[k]) {
				value = value[k];
			} else {
				// Fallback to English
				value = translations.en;
				for (const ek of keys) {
					if (typeof value === 'object' && value[ek]) {
						value = value[ek];
					} else {
						return key; // Return key if not found
					}
				}
				break;
			}
		}

		if (typeof value !== 'string') {
			return key;
		}

		// Replace parameters
		if (params) {
			for (const [param, val] of Object.entries(params)) {
				value = value.replace(`{${param}}`, String(val));
			}
		}

		return value;
	}

	/**
	 * Subscribe to language changes
	 */
	subscribe(listener: () => void): () => void {
		this.listeners.add(listener);
		return () => this.listeners.delete(listener);
	}

	private notifyListeners(): void {
		this.listeners.forEach((listener) => listener());
	}

	/**
	 * Get available languages
	 */
	getAvailableLanguages(): { code: Language; name: string }[] {
		return [
			{ code: 'en', name: 'English' },
			{ code: 'tr', name: 'Türkçe' },
			{ code: 'de', name: 'Deutsch' },
		];
	}
}

// Export singleton
export const i18n = new I18n();
export const t = (key: string, params?: Record<string, string | number>) => i18n.t(key, params);
export default i18n;
